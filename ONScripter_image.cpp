...
char *p;
for (p = filename; p <= ext; ++p) {
	filename2[p - filename] = filename[p - filename];
}
strcat(filename2, "bmp");
if (ext && (!strcmp(ext+1, "JPG") || !strcmp(ext+1, "jpg")) && IMG_isJPG(src)){ 
//not support JPG in MIYOO A30, so IMG_isJPG will always return false
utils::printError(" *** IMG_LoadJPG_RW [%s]\n", filename);
tmp = IMG_LoadJPG_RW(src);
} else {
utils::printError(" *** IMG_Load [%s->%s]\n", filename, filename2);
tmp = IMG_Load(filename2);
}
#else
